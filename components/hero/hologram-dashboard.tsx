...
export function HologramDashboard() {
  return (
    <div className="relative mx-auto aspect-square w-full max-w-[520px]">
      ...
      <div className="ring-one absolute inset-12 rounded-full border border-dashed border-cyber-cyan/55" />
      <div className="ring-two absolute inset-24 rounded-full border border-dashed border-cyber-violet/70" />
      <div className="absolute inset-[34%] grid place-items-center border border-cyber-cyan bg-black/80 shadow-[0_0_50px_rgba(0,229,255,0.22)]">
        <Cpu className="text-cyber-cyan" size={44} />
      </div>
      ...
      <div className="absolute bottom-6 right-8 flex h-24 items-end gap-1 border border-cyber-cyan/25 bg-black/50 p-3">
        {Array.from({ length: 16 }).map((_, index) => (
          <span
            key={index}
            className="signal-bar h-full w-1 origin-bottom bg-cyber-cyan"
...
function Panel({
  ...
  return (
    <div className={`absolute min-w-36 border border-cyber-cyan/40 bg-black/70 p-3 ${className}`}>
